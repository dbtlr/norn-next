...
/// Called once per stream start, after every historical event has been handed
/// to the event handler.
pub type HistoryDone = Arc<dyn Fn() + Send + Sync>;

/// The identifier of the most recent event this host recorded.
///
/// Identifiers come from one per-host source and increase across every attached
/// volume, so one identifier is a boundary for a stream whose paths span more
/// than one volume.
pub fn current_event_id() -> fs::FSEventStreamEventId {
...
impl FsEventWatcher {
    ...
    /// Create a watcher whose stream replays every event this host recorded
    /// after `since_when`, and which calls `history_done` once that replay is
    /// complete.
    ///
    ...
    pub fn with_event_history<F: EventHandler>(
        event_handler: F,
        since_when: fs::FSEventStreamEventId,
        history_done: impl Fn() + Send + Sync + 'static,
    ) -> Result<Self> {
        Self::from_parts(
            Arc::new(Mutex::new(event_handler)),
            since_when,
            Arc::new(history_done),
        )
    }
    ...
    fn run(&mut self) -> Result<()> {
        ...
        let context = Box::into_raw(Box::new(StreamContextInfo {
            ...
            history_done: self.history_done.clone(),
        ...
        // A stream the system declined to create is a failure to report, not a
        // pointer to schedule: scheduling it faults. The context is owned by
        // the stream once one exists, so with no stream it is reclaimed here.
        if stream.is_null() {
            drop(unsafe { Box::from_raw(context) });
            return Err(Error::generic(
                "FSEventStreamCreate returned no stream for the requested paths",
            ));
        }
...
unsafe fn callback_impl(
    ...
    for p in 0..num_events {
        ...
        // The sentinel that closes the replay of historical events. It carries
        // no filesystem fact — Apple specifies its path as meaningless — so it
        // is reported through its own callback and never as an event. Every
        // historical event precedes it in this same delivery order, so a
        // caller observing it knows the replay reached the handler first.
