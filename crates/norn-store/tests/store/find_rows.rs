...
use norn_store::{
    ...
    NESTED_ROW_CEILING, Nested, NestedRows, ReadRefusal, SnapshotReader, Span, Store, TagFact,
    TagSource,
};
use norn_wire::{
...
fn a_typed_cursor_refuses_after_a_re_pin_and_a_raw_one_survives_it() {
    ...
    assert_eq!(
        ...
        ReadRefusal::OrderChanged(CursorOrderChanged::new(
            "schema-1",
    ...
    assert_eq!(
        ...
        ReadRefusal::OrderChanged(CursorOrderChanged::minted_raw(Some("schema-2".to_string())))
    );
...
fn a_cursor_minted_in_another_order_than_the_requests_is_refused() {
    ...
    for (params, cursor, changed) in [
        ...
        assert_eq!(
            ...
            ReadRefusal::OrderChanged(changed),
...
fn a_link_or_finding_column_is_refused_by_name() {
    ...
    for (column, named, reads) in [
        ...
        assert_eq!(refusal, ReadRefusal::NotProjected { column: named });
