...
impl<O: EntryOps> Host<O> {
    ...
    /// `vault status` naming no vault answers, the engines are read off the
    /// same statuses, and the sanity pass reads the roots of the same
    ...
    /// each vault's last attachment met among them.
    ///
