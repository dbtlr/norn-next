...
pub trait SnapshotSource: Send + 'static {
    ...
    /// slot the publication was half way through filling — while the coverage
...
pub trait EntryOps: Send + Sync + 'static {
    ...
    /// Read and validate the two authored vault control files and answer what
    /// [`EntryOps::reload`] would decide about them, at the fingerprints it
    /// would put into service, changing nothing. The default reports
    /// unsupported, as the default reload does.
    fn judge_reload(
        &self,
        _: &VaultName,
        _: &Self::Attachment,
    ...
    /// Give back whatever this implementation keeps for a vault beside the
    /// attachment, where the attachment itself is gone.
    ///
    /// A leg that unwinds drops its attachment on the unwinding stack, so
    /// [`EntryOps::detach`] never runs for it and anything held outside the
    /// attachment — keyed by the vault's name rather than owned by the
    ...
    /// Record `registration` in the registry file this host was started from.
    ///
    ...
    ///
    /// **A name the file already records is refused, and the file is left as
    /// it is.** The file is the durable record, and a registration another
    /// writer put there since startup is that writer's, not one to replace.
    ///
    /// The default keeps no registry file, and refuses every change.
    fn record(&self, registration: &Registration) -> Result<(), RecordRefusal> {
        let _ = registration;
        Err(RecordRefusal::Unwritable(RegistryUnwritable::new(
            "this host keeps no registry file to write",
        )))
    }
    /// Retire `registration`: take it out of the registry file, and discard
    /// the derived state its vault leaves unless `keep_state` keeps it.
    ///
    ...
    /// nothing changes. Under the lock the ops give back what they hold for
    /// the vault by name, as [`EntryOps::discard`] does, and the file is read:
    ...
    ///
    /// The default keeps no registry file, and refuses every retirement.
    fn retire(
        &self,
        registration: &Registration,
        keep_state: bool,
        ...
        let _ = (registration, keep_state, standing);
        Err(RetireRefusal::Unrecorded(RegistryUnwritable::new(
            "this host keeps no registry file to write",
        )))
    }
    /// Amend `current` to `amended` in the registry file: the same name, and
    /// the fields `amended` holds.
    ///
    /// **Where the root moves, the amendment is a retirement of the derived
    /// state as well.** That state is keyed by the name and was derived from
    /// the old root, so it goes, under the vault's maintainer lock and in the
    /// order [`EntryOps::retire`] takes: the lock first, refusing as that
    /// retirement refuses; then the file read, a file this build cannot read
    /// refusing before anything is discarded; then the discard, sparing the
    /// roots [`EntryOps::retire`] spares and `amended`'s own; then the write.
    /// Where the root stands, the file is written and nothing else.
    ///
    /// The caller keeps every door of its own off the vault before this runs,
    /// as it does for a retirement.
    ///
    /// The default keeps no registry file, and refuses every amendment.
    fn amend(
        &self,
        ...
        let _ = (current, amended, standing);
        Err(RetireRefusal::Unrecorded(RegistryUnwritable::new(
            "this host keeps no registry file to write",
        )))
    }
...
/// the vocabulary spells. A third names an entry out of service while a
/// registration change holds it, and answers through that same mapping.
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum Demand {
    State(TrustState),
    MaintainerContended(MaintainerIdentity),
    ...
    UnknownVault,
    /// A registration change of the vault is under way — an unregistration,
    /// or an edit of its registration — and the entry asked is out of
    /// service until that change commits or is refused.
    EntryHeld,
    /// The mode the demand named, which this host holds no lifecycle for.
    UnsupportedMode(AttachMode),
}

/// Why a door finds no entry in service under a name: the set serves none, or
/// a registration change holds the one it serves.
///
/// These are the two demands every door answers a name with before it reads
/// anything of an entry, typed apart from the rest so a door that has nothing
/// else to answer — a status that reports rather than refuses everything
/// else — holds a refusal and cannot hold a state. Each is the [`Demand`] of
/// the same name, and renders through the one mapping that demand does.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) enum Unserved {
    /// The set serves no entry under the name.
    UnknownVault,
    /// A registration change — an unregistration, or an edit of the
    /// registration — holds the entry until the change commits or is refused.
    EntryHeld,
}

impl From<Unserved> for Demand {
    fn from(unserved: Unserved) -> Self {
        match unserved {
            Unserved::UnknownVault => Demand::UnknownVault,
            Unserved::EntryHeld => Demand::EntryHeld,
        }
    }
}

/// What parks an entry: the three facts that keep it from re-attaching,
/// whatever trust state stands beside them.
///
/// Each is the [`Demand`] of the same name, which is what every door answers
/// a parked entry with, and that demand renders through this park's own
/// answer: a registration edit refused under a park answers the envelope a
/// demand and a status answer it with.
#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) enum StandingPark {
    /// Another process holds the vault's maintainer lock.
    MaintainerContended(MaintainerIdentity),
    /// More than one registered name reaches the vault's root.
    DuplicateRoot(AliasConflict),
    /// The registry's own account of a root it cannot read.
    IdentityRefused(String),
}

impl From<StandingPark> for Demand {
    fn from(park: StandingPark) -> Self {
        match park {
            StandingPark::MaintainerContended(incumbent) => Demand::MaintainerContended(incumbent),
            StandingPark::DuplicateRoot(conflict) => Demand::DuplicateRoot(conflict),
            StandingPark::IdentityRefused(refusal) => Demand::IdentityRefused(refusal),
        }
    }
}
...
pub(crate) struct Observation {
    /// The registration the entry serves.
    pub(crate) registration: Registration,
    /// What the entry publishes: the park it stands on first, and its trust
    /// state where nothing parks it.
    pub(crate) published: Demand,
    /// The retained status facts.
    pub(crate) inspection: VaultInspection,
...
struct EntryState<A: SnapshotSource> {
    ...
    /// a wider fact: [`Job::Detach`] takes the coverage and pins nothing, and
    /// every leg past its own epilogue is still recorded as holding what it is
    /// handing to [`EntryOps::detach`]. A cleanup reading custody would give
    /// back a pin the panicked leg never took — a pin that is a read's, and one
    /// [`EntryState::unpin`] saturating away would be worse still.
    ///
    ...
    /// withdrawn by a registration change, or let go of by the set.
    ///
    ...
    /// maintainer lock, while a registration change holds that lock — and what
...
enum Service {
    ...
    Served,
    /// A registration change holds the entry — an unregistration, or an edit
    /// of its registration — and has neither committed nor been refused. Set
    /// by `Withdrawal` under the gate hold that finds the entry holding
    /// nothing and held by nothing, and put back to [`Service::Served`] by a
    /// refusal or an unwind.
    ///
    /// An entry an edit replaced stays here once the edit commits: the set
    /// serves its successor under the name, and a caller still holding this
    /// entry is answered as held and reaches the successor when it asks again.
    Withdrawn,
...
impl<A: SnapshotSource> EntryState<A> {
    ...
    /// gate, so every other holder of that entry waits behind it. The bound is
    /// the read-only open's busy timeout, and it is per statement rather than
    /// per open: the open runs two statements that read the database — the
    /// journal-mode read and the store-epoch read that binds the connection to
    /// its file — so a mint that met a busy at each waits that timeout twice.
    /// It is paid by a read that is otherwise about to be refused, and there
    ...
    fn parked(&self) -> Option<Demand> {
        self.standing_park().map(Demand::from)
    }

    /// [`EntryState::parked`]'s park, typed apart from the demands that are
    /// no park.
    fn standing_park(&self) -> Option<StandingPark> {
        self.maintainer_contended
            .clone()
            .map(StandingPark::MaintainerContended)
            .or_else(|| self.duplicate_root.clone().map(StandingPark::DuplicateRoot))
            .or_else(|| {
                self.identity_refused
                    .clone()
                    .map(StandingPark::IdentityRefused)
            })
    }
    ...
    /// Why this entry is out of service, and nothing while it is served.
    ///
    /// **This is the one place that answer is decided.** An entry a
    /// registration change holds answers as held until the change commits or
    /// is refused, so a refused change leaves nothing a caller could have
    /// observed but a wait; an entry the set let go of answers as a name the
    /// set does not serve. Every door answers with it: a demand, a retry, a
    /// completion, a status read and a read as the demand
    /// [`EntryState::withdrawal_answer`] makes of it, a reload as
    /// [`ReloadRefusal::Unavailable`] carrying that demand, each inspection
    /// door as its refusal, and `vault status` as this refusal itself.
    fn out_of_service(&self) -> Option<Unserved> {
        match self.service {
            Service::Served => None,
            Service::Withdrawn => Some(Unserved::EntryHeld),
            Service::Retired => Some(Unserved::UnknownVault),
        }
    }

    /// What every door that answers in demands answers for this entry out of
    /// service — [`EntryState::out_of_service`] as the demand of the same
    /// name — and nothing while it is served.
    fn withdrawal_answer(&self) -> Option<Demand> {
        self.out_of_service().map(Demand::from)
    }
    ...
    fn observe(&self, registration: &Registration) -> Result<Observation, Unserved> {
        ...
        Ok(Observation {
            ...
            inspection: VaultInspection {
                ...
                reader_unavailable: self.reader_unavailable.clone(),
...
enum Job {
    ...
    /// A dry run of a reload: judge the candidate and activate nothing.
...
/// Unattached yet: Unattached is what an entry publishes once its resources are
/// no longer its own, and a caller that waits for it is waiting for exactly
/// that. On every leg that returns, that instant is [`EntryOps::detach`]
/// returning, having given the watcher, the store and the maintainer lock back.
/// A leg that unwinds reaches no detach — its attachment was dropped on the
/// unwinding stack, which is where an implementation's own `Drop` released it —
/// so Unattached there says the entry holds nothing rather than that a detach
/// ran, and [`EntryOps::discard`] is what gives back the residue no detach was
/// there to. Warming is the state meaning attached and not readable, and the
/// phase beside it names the leg — resources going back, nothing counted.
///
...
/// How a release ends: whether the lease standing at its end is answered by the
/// release itself, or by the demand that comes after it.
///
/// The two tails part on what the release is the end of. A teardown is a
/// decision the entry took — an idle detach, a refusal, a failure verdict — and
/// the lease it outlived asked for coverage the teardown gave back, so the
/// release is what owes it and the re-attach it sends is the answer. A leg that
/// unwound decided nothing: the run is over part way through, and the work a
/// re-arm would send is the same work under the same conditions, with nothing
/// between the two runs to make the second end differently. A lease that merely
/// stands is not a request for that.
#[derive(Clone, Copy)]
enum ReleaseTail {
    /// Answer the lease standing at the release's end: the entry warms into an
    /// attach the release dispatches under its own claim.
    HonorDemand,
    /// Answer no lease. The entry ends Unattached — or under the park that is a
    /// wider fact than any label beneath it — and the next [`Host::demand`]
    /// schedules the attach through the door every unattached entry is served
    /// by. One demand call therefore buys one attempt, whatever leases stand.
    LeaveToTheNextDemand,
}
...
///
/// The lease standing at the end of a release that honors one is answered by
/// the release itself, because the release is what the lease was waiting
/// behind. A parked entry owes it nothing, and neither does one owing a
/// recovery no live lease asked for. The park outlives the publication below:
/// Unattached says the resources are back, and the lease still reports the park
/// that is why nothing follows it. The requirement is read before it is
/// cleared: a lease that asked for a recovery is asking for the coverage the
/// re-attach below installs.
///
...
///
/// Which of the two tails the release ends on is the caller's to say, and
/// [`ReleaseTail`] is where the two are set out.
...
/// Take the gate an acquisition claims a root identity under, recovering it
/// where a panic was taken while it stood held.
///
/// The poison is tolerated at every site rather than at some of them, and the
/// map is what makes that sound: it is a plain table from identity to the vault
/// holding the claim, and every write is one whole insert, one whole remove or
/// one whole sweep taken under this lock. No move here leaves it half written,
/// so an unwind crossing the guard leaves the table exactly as the last
/// completed write left it, and the poison flag says only that some stack
/// unwound past a hold — a fact about a leg, not about the table.
///
/// Propagating the poison instead would put a panic inside the unwind handler.
/// [`reclaim_unwound_leg`] sweeps this map, and it runs on the worker that
/// caught the unwind or on the dispatcher thread that caught a poll's: a panic
/// there ends the thread that was cleaning up, takes the worker slot or the
...
/// An entry a registration change has withdrawn from service, put back in
/// service when this is dropped unless the change committed.
///
/// Dropping is every way out of the change that did not commit — a refusal
/// and an unwinding registry change alike — so no path leaves the entry
/// answering as withdrawn while the set still serves it.
struct Withdrawal<'a, A: SnapshotSource> {
    entry: &'a Entry<A>,
    committed: bool,
}

impl<'a, A: SnapshotSource> Withdrawal<'a, A> {
    /// Withdraw `entry` from service, in the one gate hold that finds it
    ...
    /// Keep the entry withdrawn: the change is written, and the move on the
    /// set that follows takes the entry out of it as it stands.
    fn commit(mut self) {
        self.committed = true;
    }
}

impl<A: SnapshotSource> Drop for Withdrawal<'_, A> {
    fn drop(&mut self) {
        if self.committed {
            return;
        }
...
/// Take the lock every registration change holds, recovering it where a
/// change unwound while it stood held.
///
/// The lock guards no data: it orders the changes, and each change reads and
/// writes the serving set and the registry file under their own locks. A
/// change that unwound left nothing half written here, so its poison says only
/// that a stack unwound past a hold, and the next change goes on.
fn registration_gate<O: EntryOps>(shared: &Shared<O>) -> MutexGuard<'_, ()> {
    shared
        .registration_gate
        .lock()
        .unwrap_or_else(PoisonError::into_inner)
...
/// [`Claim::begin_poll`] write it blind, so a leg beginning over another's
/// registration takes it over; what keeps that from happening under a running
...
/// either would take a pin a read in flight is holding.
///
...
///
/// **What the ops keep beside an attachment is given back where the attachment
/// is not.** An attachment that went with the unwinding stack reaches
/// [`EntryOps::detach`] never, and per-vault residue the ops hold outside it —
/// keyed by name rather than owned by the attachment — would outlive an entry
/// that publishes as holding nothing. [`EntryOps::discard`] is what gives that
/// back, and it is called exactly where no coverage goes to the detach.
///
/// **The release answers no lease, and the next demand does.** Re-arming here
/// would send the work that just unwound again, under the conditions that
/// unwound it, with nothing between the two runs — and a lease that merely
/// stands would then buy an unbounded run of panicking legs rather than one
/// answer. [`ReleaseTail::LeaveToTheNextDemand`] is that stance, and
/// [`Host::demand`] is the door: it schedules over an entry standing at
/// Unattached or Untrusted with no claim held and no detach in flight, which is
/// exactly what this leaves.
///
/// **The reason is published over what the release left, and over nothing
/// else.** A park is a wider fact than the label beneath it, and it is not
...
fn reclaim_unwound_leg<O: EntryOps>(
    ...
    if attachment.is_none() {
        ...
        // still held its own coverage the release below is that way through,
        // and the detach it reaches gives the same residue back itself.
        shared.ops.discard(name);
    }
    finish_release(
        shared,
        &entry,
        ...
        epoch,
        attachment,
        ReleaseTail::LeaveToTheNextDemand,
    );
    let mut state = entry.gate.lock().expect("entry gate poisoned");
...
fn park_on_current_classification<O: EntryOps>(shared: &Arc<Shared<O>>, name: &VaultName) {
    let _attach_guard = attach_gate(shared);
    park_on_classification_under_gate(shared, name);
}

/// [`park_on_current_classification`]'s read and park, for a caller already
/// holding the attach gate.
fn park_on_classification_under_gate<O: EntryOps>(shared: &Arc<Shared<O>>, name: &VaultName) {
...
///
/// A departure raises nothing: taking a name out of an alias group only
/// shrinks it. What it owes is the other side — the names left parked on a
/// conflict the departed name was part of — and [`reclassify_after_departure`]
/// is that read.
fn serve<O: EntryOps>(
    shared: &Arc<Shared<O>>,
    registration: Registration,
...
/// Serve `amended` in place of the entry standing under its name, which a
/// registration edit has withdrawn and committed, and answer what the new
/// entry publishes.
///
/// The new entry joins as every entry joins — unattached, holding nothing —
/// at an epoch past every job the old one left. It holds none of what the old
/// one's attachments recorded: no delivered engine, no store reading and no
/// advisory, so a status reads it undelivered and advising nothing until its
/// own attach publishes them. Where the edit moved the
/// root, the join classifies the new root as [`serve`] classifies a root
/// that arrives, and the names that were parked on a conflict with the old
/// root are classified again as after a departure. Where the root stands,
/// nothing about any root has changed, and nothing is read.
fn serve_in_place<O: EntryOps>(
    shared: &Arc<Shared<O>>,
    amended: Registration,
    moved: bool,
) -> Demand {
    let name = amended.name.clone();
    shared.entries.replace(amended).expect(
        "the registration lock holds every change to the set, and the entry it replaces is \
         withdrawn under it",
    );
    if moved {
        reclassify_after_departure(shared, &name);
        park_on_current_classification(shared, &name);
    }
    shared
        .entries
...
/// Refuse `identity` where a served vault other than `name` reaches it,
...
/// Classify again every served root parked on a conflict `departed` was part
/// of, now that the set no longer serves it.
///
/// A departure only shrinks an alias group, so this withdraws parks and
/// raises none but the one a smaller group still earns: each such name's
/// conflict park is withdrawn and its root read again under the attach gate,
/// which parks it on whatever that read still finds — the rest of the group,
/// or a root it cannot read — and leaves a name nothing else reaches free to
/// serve at once. A lease standing over a name left free is answered the way
/// every release answers one, with the work it is owed scheduled here.
///
/// A park this departure did not raise is not this read's to withdraw: a
/// conflict naming only others stands until an acquisition reads it.
fn reclassify_after_departure<O: EntryOps>(shared: &Arc<Shared<O>>, departed: &VaultName) {
        ...
        for entry in shared.entries.snapshot() {
            {
                ...
                if !state
                    .duplicate_root
                    .as_ref()
                    .is_some_and(|conflict| conflict.aliases().names().contains(departed))
                {
                    continue;
                }
                state.duplicate_root = None;
            }
            park_on_classification_under_gate(shared, entry.name());
            let mut state = entry.gate.lock().expect("entry gate poisoned");
            if schedule_demanded_work(&mut state, entry.name()).is_some() {
                drop(state);
                owed.push(entry);
            }
        }
...
impl<O: EntryOps> Host<O> {
    ...
    /// the authority, and what it finds is in the demand this answers.
    pub(crate) fn register(
        ...
        shared
            ...
            .map_err(|refused| match refused {
                RecordRefusal::AlreadyRecorded => RegistrationRefusal::AlreadyRecorded,
                RecordRefusal::Unwritable(unwritable) => {
    ...
    /// Take `name` out of the registry file and the serving set, retiring the
    ...
    /// serve is unknown. **The entry is withdrawn from service under its own
    /// gate, in the hold that finds it holding nothing and held by nothing**,
    ...
    /// entry held, which a refusal leaves true of the past and nothing more.
    /// Whatever a discard did take before a refusal is derived state, which
    /// the next attach derives again from the vault.
    pub(crate) fn unregister(
        ...
        let withdrawal = Withdrawal::begin(&entry)?;
        let standing = shared.entries.registrations();
        shared
            .ops
            .retire(&entry.registration, keep_state, &standing)?;
        withdrawal.commit();
        drop(entry);
        shared
            .entries
            .remove(name)
            .expect("an entry withdrawn under the registration lock leaves the set as it stands");
        reclassify_after_departure(shared, name);
        Ok(())
    }

    /// Amend `name`'s registration in the registry file and the serving set
    /// by `edit`, and answer the registration as amended beside what its
    /// entry publishes.
    ///
    /// Everything runs under the registration lock, and in this order. A name
    ...
    /// file records the amendment does the set serve a fresh entry for it in
    /// place of the withdrawn one, at an epoch past every job the withdrawn
    /// one left. A root that moved is classified at that join, and the names
    /// parked on a conflict with the old root are classified again.
    ///
    /// The file is written before the set changes, so the entry that serves
    /// the amendment is never one the file does not record. **A refusal puts
    ...
    pub(crate) fn set(
        ...
        let standing = shared.entries.registrations();
        shared.ops.amend(current, &amended, &standing)?;
        withdrawal.commit();
        let moved = amended.root != current.root;
        drop(entry);
        let published = serve_in_place(shared, amended.clone(), moved);
        Ok((amended, published))
    }
    ...
    /// The retained internal status facts for one served vault: the trust
    /// state beside the retained reload diagnostics, the read seam's refusal
    /// and the advisories its last attachment met.
    ///
    /// They are read out of the same observation of the entry — one hold of
    /// its gate — that `vault status` answers from, so what this reports and
    /// what a status reports are one reading.
    ///
    ...
    ///
    /// The drift reading `vault status` reports — current, reload pending,
    /// unreadable, or inactive while the entry holds no active fingerprints
    /// yet — computed on demand because watcher events activate neither
    /// control file. A status measures it over the controls its own
    /// observation of the entry read, and this answers through the same
    /// measurement.
    pub fn authored_drift(&self, name: &VaultName) -> Result<AuthoredDrift, Demand> {
        let observation = self.observe(name).map_err(Demand::from)?;
        Ok(AuthoredDrift::of(
            &observation.registration,
            observation.inspection.active_fingerprints,
            observation.control_root.as_deref(),
        ))
    }

    /// What the entry serving `name` stands at, under one hold of its gate,
    /// or why no entry in service answers for the name.
    pub(crate) fn observe(&self, name: &VaultName) -> Result<Observation, Unserved> {
        let entry = self
            .shared
            .entries
            .get(name)
    ...
    /// left out, as a listing taken after the removal leaves it out. One an
    /// edit replaced in that window is observed held, as every caller still
    /// holding it is answered; a snapshot holds one entry per name, so the
    /// entry replacing it is not observed beside it. Each
    ...
    ///
    /// [`Host::vault_reload`] answers the `vault reload` verb through this.
    /// Watcher events activate neither control file, so an edited schema
    /// reaches the store's pin only through this reload or through the fresh
    /// read an attach or a recovery leg makes for itself.
    pub fn reload(&self, name: &VaultName) -> Result<ReloadJudgment, ReloadRefusal> {
        self.request_reload(name, Job::Reload)
    }

    /// Request one judgment of the vault's reload candidate and wait for it:
    /// what a reload asked now would decide, at the fingerprints it would put
    /// into service, with nothing put into service.
    ///
...
fn end_job_leg<O: EntryOps>(
    ...
    if releasing {
        finish_release(shared, entry, name, epoch, held, ReleaseTail::HonorDemand);
        return;
    }
...
mod tests {
    ...
    struct FakeOps {
        ...
        /// The whole registration the latest attach under each name was
        /// handed: its root, its schema source and its watch backend.
        attach_registrations: Mutex<BTreeMap<VaultName, Registration>>,
        ...
        /// The one vault whose attach panics, so the vault beside it still
        /// attaches on the same pool.
        panic_in_attach_at: Mutex<Option<VaultName>>,
        /// The one vault whose detach panics. `detach` is where the watcher,
        /// the store and the maintainer lock go back, and two production sites
        /// reach it while holding the attach gate — so this is the arming a
        /// case poisons that gate with.
        ...
        /// Every retirement that went through, as the vault and whether its
        /// derived state was discarded, in call order.
        retirements: Mutex<Vec<(VaultName, bool)>>,
        /// Where a case arranged one, the rendezvous every registry write
        /// waits at for a second writer before it writes.
        record_overlap: Mutex<Option<Arc<Overlap>>>,
        /// Where a case arranged one, the pause every retirement and every
        /// amendment takes before it reads the registry file.
        ...
        /// Every amendment that went through, as the registration it amended
        /// and the one it left, in call order.
    ...
    ///
    /// Each writer waits a bounded while for the other: a second writer the
    /// host keeps out never arrives, and the first goes on alone once the
    /// wait is over.
    #[derive(Default)]
    struct Overlap {
        writers: Mutex<usize>,
    ...
    impl Overlap {
        /// How long a writer waits for the other.
        const WAIT: Duration = Duration::from_millis(500);

        fn meet(&self) {
            let mut writers = self.writers.lock().expect("overlap poisoned");
            *writers += 1;
            self.arrived.notify_all();
            let _ = self
                .arrived
                .wait_timeout_while(writers, Self::WAIT, |writers| *writers < 2)
                .expect("overlap poisoned");
        }
    }

    /// A retirement held under the maintainer lock: the retiring thread and
    /// the case meet at `entered`, and the retirement goes on once they meet
    /// again at `resume`.
    struct RetirePause {
    ...
    impl EntryOps for Arc<FakeOps> {
        ...
        fn attach(
            ...
            self.attach_registrations
                .lock()
                .expect("attach registrations poisoned")
            ...
            if self.panic_in_attach.load(Ordering::SeqCst)
                || stands_for_the_vault(&self.panic_in_attach_at, &registration.name)
            {
                panic!("{ATTACH_PANIC}");
            }
        ...
        fn poll(
            ...
            *self
            ...
            if stands_for_the_vault(&self.panic_in_poll_at, name) {
                panic!("{POLL_PANIC}");
            }
        ...
        fn detach(&self, name: &VaultName, _: FakeCoverage) {
            if stands_for_the_vault(&self.panic_in_detach_at, name) {
                panic!("{DETACH_PANIC}");
            }
        ...
        fn record(&self, registration: &Registration) -> Result<(), RecordRefusal> {
            ...
            if self.registry_unwritable.load(Ordering::SeqCst) {
                return Err(RecordRefusal::Unwritable(RegistryUnwritable::new(
                    "the fake registry file is read-only",
                )));
            }
            let mut registry = self.registry.lock().expect("registry poisoned");
            if registry.contains_key(&registration.name) {
                return Err(RecordRefusal::AlreadyRecorded);
            }
        ...
        fn retire(
            ...
            if self.panic_in_retire.load(Ordering::SeqCst) {
                panic!("{RETIRE_PANIC}");
            }
            let pause = self.retire_pause.lock().expect("pause poisoned").clone();
            if let Some(pause) = pause {
                pause.entered.wait();
                pause.resume.wait();
            }
            if self.registry_unwritable.load(Ordering::SeqCst) {
                return Err(RetireRefusal::Unrecorded(RegistryUnwritable::new(
                    "the fake registry file is read-only",
                )));
            }
        ...
        /// A root move meets the maintainer another process is arranged to
        /// hold and the data directory arranged to keep its state, as a
        /// retirement does; an amendment that leaves the root standing meets
        ...
        fn amend(
            ...
            let moves = current.root != amended.root;
            if moves
            ...
            if self.panic_in_amend.load(Ordering::SeqCst) {
                panic!("{AMEND_PANIC}");
            }
            if self.registry_unwritable.load(Ordering::SeqCst) {
                return Err(RetireRefusal::Unrecorded(RegistryUnwritable::new(
            ...
            if moves && self.discard_refused.load(Ordering::SeqCst) {
                return Err(RetireRefusal::Undiscarded(
                    "the fake data directory keeps its state".into(),
                ));
            }
            self.registry
                .lock()
                .expect("registry poisoned")
                .insert(amended.name.clone(), amended.clone());
            self.amendments
                .lock()
                .expect("amendments poisoned")
                .push((current.clone(), amended.clone()));
            Ok(())
        }
    }

    /// The vaults the ops were told to give per-vault residue back for.
    fn discards_of(ops: &FakeOps, name: &VaultName) -> usize {
        ops.discards
            .lock()
            .expect("discards poisoned")
            .iter()
            .filter(|discarded| *discarded == name)
            .count()
    }
    ...
    fn stands_for_the_vault(arranged: &Mutex<Option<VaultName>>, name: &VaultName) -> bool {
        arranged
            .lock()
            .expect("an arranged vault poisoned")
            .as_ref()
            == Some(name)
    }
    ...
    fn status_params(name: &VaultName) -> norn_wire::StatusParams {
        norn_wire::StatusParams::new().with_vault(norn_wire::VaultAddress::name(name.clone()))
    }

    /// Where `name` stands, as `vault status` reports it.
    fn status_of<O: EntryOps>(host: &Host<O>, name: &VaultName) -> norn_wire::VaultStatus {
        match host.vault_status(&status_params(name)) {
            Ok(norn_wire::StatusReport::Vault { status, .. }) => *status,
            other => panic!("the status of {name} answered {other:?}"),
        }
    }

    /// What every entry adds up to, as `vault status` naming no vault reports
    /// it.
    fn rolled_up<O: EntryOps>(host: &Host<O>) -> norn_wire::RollUp {
    ...
    /// The detail `vault status` refused `params` with.
    fn status_refused<O: EntryOps>(
        host: &Host<O>,
        params: &norn_wire::StatusParams,
    ) -> ErrorDetail {
        host.vault_status(params)
            .expect_err("the status was answered")
            .detail()
            .clone()
    }

    /// Whether `name`'s entry stands as nothing asked anything of it: no
    /// demand lease recorded, no claim on its work and no job waiting for
    /// its slot.
    fn nothing_asked_of<O: EntryOps>(host: &Host<O>, name: &VaultName) -> bool {
        let entry = host.shared.entries.get(name).expect("the vault is served");
        let state = entry.gate.lock().unwrap();
        state.demand_leases == 0 && !state.claim.is_held() && !state.claim.slot_taken()
    }
    ...
    fn the_status_of_an_unattached_entry_answers_without_attaching_it() {
        ...
        assert_eq!(status.registration.name, name);
        assert_eq!(
            status.published,
            norn_wire::Published::state(TrustState::Unattached)
        );
    ...
    /// **A ready entry reports ready, and its lease is the only demand on
    /// it**: the status added none.
    #[test]
    fn the_status_of_a_ready_entry_reports_ready_and_adds_no_demand() {
        ...
        assert_eq!(
            status.published,
            norn_wire::Published::state(TrustState::Ready)
        );
        assert_eq!(status.last_reload_failure, None);
        assert_eq!(status.reads_refusing, None);
        let entry = host.shared.entries.get(&name).unwrap();
        assert_eq!(entry.gate.lock().unwrap().demand_leases, 1);
        drop(lease);
    }
    ...
    /// **A warming entry reports the phase its work is in and the counts it
    /// has published.**
    #[test]
    fn the_status_of_a_warming_entry_reports_its_phase_and_counts() {
        let ops = Arc::new(FakeOps::default());
        ops.heal_in_attach.store(true, Ordering::SeqCst);
        ops.block_attach.store(true, Ordering::SeqCst);
        let (host, name) = fixture_without_ambient_polling(Arc::clone(&ops));
        ...
        assert_eq!(
            status_of(&host, &name).published,
            norn_wire::Published::state(TrustState::warming(WarmingPhase::Healing, 1, Some(2)))
        );
        ops.attach_release.store(true, Ordering::SeqCst);
        wait_for_state(&host, &name, TrustState::Ready);
    }

    /// **An entry whose derived state cannot be trusted reports the reason
    /// its trust state carries**, in band.
    #[test]
    fn the_status_of_an_untrusted_entry_reports_its_reason() {
        ...
        wait_for_state(&host, &name, untrusted.clone());

        assert_eq!(
            status_of(&host, &name).published,
            norn_wire::Published::state(untrusted)
        );
    }

    /// **An entry parked on a contended maintainer lock reports the park in
    /// its own code, naming the incumbent**, rather than refusing the status
    /// or reporting the label beneath it.
    #[test]
    fn the_status_of_an_entry_parked_maintainer_contended_reports_the_park() {
        ...
        let norn_wire::Published::Parked { refusal, .. } = status_of(&host, &name).published else {
            panic!("the parked entry did not report its park");
        };
        assert_eq!(
            refusal.detail(),
    ...
    /// **An entry parked on a duplicate root or on a root the registry
    /// cannot read reports that park in its own code**, the detail its own
    /// refusal carries.
    #[cfg(unix)]
    #[test]
    fn the_status_of_an_entry_parked_by_the_registry_reports_the_park() {
        for park in [Park::Identity, Park::DuplicateRoot] {
            let scratch = temp_base("status-parked");
            let root = scratch.root().join("root");
            let ops = Arc::new(FakeOps::default());
            let name = VaultName::new("notes").unwrap();
            ...
            let norn_wire::Published::Parked { refusal, .. } = status_of(&host, &name).published
            else {
                panic!("the entry parked on {park:?} did not report its park");
            };
            assert_eq!(refusal.detail(), &parked, "{park:?}");
            assert!(nothing_asked_of(&host, &name), "{park:?}");
        }
    }

    /// **The reload failure an entry retains is reported beside where it
    /// stands**, named by the control-file failure a refused reload names it
    /// by, and a roll-up names the vault for it.
    #[test]
    fn the_status_reports_the_retained_reload_failure() {
        let ops = Arc::new(FakeOps::default());
        ops.reload_supported.store(true, Ordering::SeqCst);
        ops.reload_schema_changed.store(true, Ordering::SeqCst);
        ops.reload_schema_apply_failure
        ...
        let reported = crate::refusal::control_file_failure(&failure);
        assert_eq!(
            status_of(&host, &name).last_reload_failure,
            Some(reported.clone())
        );
        assert!(
            rolled_up(&host)
                .attention()
                .contains(&norn_wire::Attention::reload_failed(name.clone(), reported)),
            "the roll-up did not name the failed reload"
        );
    }

    /// **An entry serving every surface but its reads reports why its reads
    /// refuse**, beside a trust state the refusal did not move.
    #[test]
    fn the_status_reports_why_an_entrys_reads_refuse() {
        ...
        assert!(status.reads_refusing.is_some(), "{status:?}");
    }

    /// **A name the host serves nothing under is refused as unknown**, and a
    ...
    fn a_status_refuses_an_unknown_name_and_a_root() {
        ...
        assert_eq!(
            status_refused(
                &host,
                &norn_wire::StatusParams::new().with_vault(norn_wire::VaultAddress::root(root))
            ),
            ErrorDetail::unsupported_attach_mode(AttachMode::Throwaway)
        );
    }

    /// **A host serving nothing rolls up to nothing**: no vault counted and
    ...
    fn a_roll_up_counts_mixed_entries_and_names_what_wants_attention() {
        let [ready, idle, parked, untrusted] =
            ["ready", "idle", "parked", "untrusted"].map(|name| VaultName::new(name).unwrap());
        let ops = Arc::new(FakeOps::default());
        ...
        ops.withholds_trust.store(true, Ordering::SeqCst);
        let _untrusted_lease = host.demand(&untrusted, AttachMode::Durable).unwrap();
        let reason = UntrustedReason::schema_unreadable("this fake withholds trust");
        ...
        *ops.advisories.lock().unwrap() = vec![linked("away.md")];
        ...
        let _parked_lease = host.demand(&parked, AttachMode::Durable).unwrap();
        wait_for_state(&host, &parked, TrustState::Ready);
        let conflict = conflict_over(&parked);
        ...
        assert_eq!(
            ...
            [
                ...
                norn_wire::Attention::advisory(ready.clone(), link.clone()),
    ...
    /// The envelope `vault reload` refused `params` with.
    fn reload_refused(host: &Host<Arc<FakeOps>>, params: &norn_wire::ReloadParams) -> ErrorDetail {
        host.vault_reload(params)
            .expect("the host is running")
            .expect_err("the reload was answered")
            .detail()
            .clone()
    }

    /// **A name the registry does not hold is refused as unknown, echoing the
    ...
    fn vault_reload_refuses_an_unregistered_name_as_unknown() {
        ...
        for dry_run in [false, true] {
            assert_eq!(
                reload_refused(&host, &reload_params(&unregistered, dry_run)),
                ErrorDetail::unknown_vault(unregistered.clone())
            );
        }
    }

    /// **An address naming a root asks for a throwaway attach**, which holds
    ...
    fn vault_reload_refuses_a_root_as_an_unsupported_attach() {
        ...
        for dry_run in [false, true] {
            ...
            assert_eq!(
                reload_refused(&host, &params),
                ErrorDetail::unsupported_attach_mode(AttachMode::Throwaway)
            );
        }
    }

    /// **An entry nothing has attached holds nothing to reload yet.**
    #[test]
    fn vault_reload_refuses_an_unattached_entry_as_not_ready() {
        ...
        for dry_run in [false, true] {
            assert_eq!(
                ...
                ErrorDetail::entry_not_ready(norn_wire::NotReady::unattached())
            );
        }
        assert_eq!(ops.attaches.load(Ordering::SeqCst), 0);
    }

    /// **An entry whose derived state cannot be trusted refuses with the
    /// reason its trust state carries.**
    #[test]
    fn vault_reload_refuses_an_untrusted_entry_with_its_reason() {
        let ops = Arc::new(FakeOps::default());
        ops.reload_supported.store(true, Ordering::SeqCst);
        let (host, name) = fixture(Arc::clone(&ops), Duration::from_secs(60));
        drop(host.demand(&name, AttachMode::Durable).unwrap());
        ...
        let reason = UntrustedReason::environmental_refusal("refused");
        wait_for_state(&host, &name, TrustState::untrusted(reason.clone()));
    ...
    fn vault_reload_refuses_a_ready_entry_a_job_holds_as_busy() {
        ...
        let _lease = host.demand(&name, AttachMode::Durable).unwrap();
        wait_for_state(&host, &name, TrustState::Ready);
        let holding = Arc::clone(&host);
        let held = name.clone();
        let first = thread::spawn(move || holding.reload(&held));
        wait_for_flag("reload_started", &ops.reload_started);
        ...
        ops.reload_release.store(true, Ordering::SeqCst);
        first.join().unwrap().expect("the reload holding the entry");
    }

    /// **An attachment that holds no reload at all refuses as a reload that
    /// failed, of the unsupported kind**, and stays `Ready`.
    #[test]
    fn vault_reload_refuses_an_attachment_without_a_reload_as_unsupported() {
        ...
        for dry_run in [false, true] {
            assert_eq!(
                reload_refused(&host, &reload_params(&name, dry_run)),
                ErrorDetail::reload_failed(norn_wire::ReloadFailure::unsupported())
            );
            assert_eq!(host.state(&name), answered(TrustState::Ready));
        }
    }
    ...
    fn vault_reload_answers_a_stopped_host_with_no_code() {
        for dry_run in [false, true] {
            ...
            assert!(matches!(
                host.vault_reload(&reload_params(&name, dry_run)),
                Err(HostError::WorkerStopped)
            ));
        }
    }
    ...
    /// A demand lease standing across an unwind buys the attempt it asked for
    /// and no more. The release the cleanup runs answers no lease, so a vault
    /// whose attach panics every time settles at the unwind reason and stays
    /// there while the lease stands; the next demand call is what asks again,
    /// and it is answered.
    #[test]
    fn a_lease_held_across_an_unwind_buys_one_attempt_and_the_next_demand_asks_again() {
        let ops = Arc::new(FakeOps::default());
        ...
        *ops.panic_in_attach_at.lock().unwrap() = Some(name.clone());
        let lease = host.demand(&name, AttachMode::Durable).unwrap();
        wait_for_state(&host, &name, unwound(ATTACH_PANIC));

        assert_eq!(
            ops.attaches.load(Ordering::SeqCst),
            1,
            "one demand ran more than the one attach it asked for"
        );
        {
            let state = entry.gate.lock().unwrap();
            assert!(!state.claim.is_held(), "the gate stands for nothing");
            ...
            assert!(
                state.demand_leases > 0,
                "the lease this case holds is not recorded against the entry"
            );
        }

        // The lease stands, and standing is not asking: nothing is dispatched
        // for it, by the release that ended or by a later tick.
        thread::sleep(Duration::from_millis(200));
        assert_eq!(
            ops.attaches.load(Ordering::SeqCst),
            1,
            "a lease that merely stands re-armed the leg that unwound"
        );
        assert_eq!(host.state(&name), answered(unwound(ATTACH_PANIC)));

        // The pool's one slot outlives the run, so the vault beside it attaches
        // on the same worker.
        drop(host.demand(&bystander, AttachMode::Durable).unwrap());
        wait_for_state(&host, &bystander, TrustState::Ready);

        // A fresh demand call is the door an untrusted entry is served through,
        // and it is answered — with the lease from the first attempt still held.
        *ops.panic_in_attach_at.lock().unwrap() = None;
        ...
        assert_eq!(
            ops.attaches.load(Ordering::SeqCst),
            3,
            "the second demand ran something other than one attach for z"
        );
        drop((lease, second));
    }
    ...
    /// The control for the case above: the same leg, pinning as it takes the
    /// coverage, and unwinding. The pin the entry recorded for it goes back,
    /// and the read's own pin is the one left standing.
    #[test]
    fn a_pinning_leg_that_unwinds_gives_its_own_pin_back_and_no_other() {
        ...
        let read = host.begin_read(&name).expect("the entry answers a read");
        ops.panic_in_reconcile.store(true, Ordering::SeqCst);
        ops.off_thread_poll_batches.store(1, Ordering::SeqCst);
        poll_watchers(&host.shared);
        wait_for_state(&host, &name, unwound(RECONCILE_PANIC));

        {
            ...
            assert_eq!(
                state.safety_pins, 1,
                "the reconcile leg's own pin and the read's did not part"
            );
            assert!(
                state.pinned_leg.is_none(),
                "the entry is still pinned for the leg that unwound"
            );
        }
        drop(read);
        assert!(!entry.gate.lock().unwrap().pinned());
    ...
    /// The attachment a leg was holding goes with the unwinding stack, so
    /// nothing reaches `EntryOps::detach` for it. What the ops keep beside an
    /// attachment — a per-vault engine slot, keyed by name — is given back
    /// through `EntryOps::discard` instead, so the entry publishes as holding
    /// nothing over ops that hold nothing for it.
    #[test]
    fn an_unwound_leg_gives_the_ops_their_per_vault_residue_back() {
        ...
        assert_eq!(
            discards_of(&ops, &name),
            1,
            "the leg's attachment went with the unwind and its residue stayed"
        );
        assert_eq!(
            ops.detaches.load(Ordering::SeqCst),
            0,
            "an attachment the unwind dropped reached the detach"
        );

        // A teardown whose coverage the entry still holds reaches the detach,
        // and the detach is what gives the same residue back there: the two
        // calls do not double up.
        ops.panic_in_reconcile.store(false, Ordering::SeqCst);
        drop(host.demand(&name, AttachMode::Durable).unwrap());
        wait_for_state(&host, &name, TrustState::Ready);
        ...
        assert_eq!(
            discards_of(&ops, &name),
            1,
            "a release that reached the detach discarded beside it"
        );
    }
    ...
    fn this_crate_is_built_to_unwind() {
        assert!(
            cfg!(panic = "unwind"),
            "norn-host is built to abort on panic, and the leg unwind guard catches nothing"
        );
    }
    ...
    mod registration_changes {
        ...
        ///
        /// Every registry write waits at a rendezvous for a second writer, so
        /// two registrations that both found the root unserved meet inside
        /// the write, and both would be admitted.
        #[cfg(unix)]
        #[test]
        fn concurrent_registrations_of_one_root_admit_exactly_one() {
            let ops = Arc::new(FakeOps::default());
            *ops.record_overlap.lock().unwrap() = Some(Arc::default());
        ...
        /// An entry standing on a park that nothing holds is unregistered, and
        /// the park leaves with it.
        #[test]
        fn a_parked_entry_nothing_holds_is_unregistered() {
            ...
            let report = unregister(&host, UnregisterParams::new(name.clone()))
                .expect("a parked entry nothing holds was refused");

            assert_eq!(report, UnregisterReport::new(name.clone(), true));
            assert_eq!(*ops.retirements.lock().unwrap(), [(name.clone(), true)]);
            assert_eq!(listed(&host), Vec::<VaultName>::new());
            assert_eq!(
                host.state(&name)
                    .expect_err("an unregistered vault refuses")
                    .detail(),
                &ErrorDetail::unknown_vault(name)
            );
        }

        /// Two names over one root, both parked on the conflict. Unregistering
        /// one classifies the other again at once: its park is gone, and the
        /// lease standing over it is served without being asked again.
        #[cfg(unix)]
        #[test]
        fn a_name_parked_beside_an_unregistered_one_serves_at_once() {
            let ops = Arc::new(FakeOps::default());
            let scratch = temp_base("unregister-alias");
            let root = scratch.root().join("root");
            std::fs::create_dir_all(&root).unwrap();
            let link = scratch.root().join("link");
            std::os::unix::fs::symlink(&root, &link).unwrap();
            ...
            let conflict = a_conflict([a.clone(), b.clone()]);
            drop(host.demand(&a, AttachMode::Durable).unwrap());
            wait_for_park(&host, &a, Demand::DuplicateRoot(conflict.clone()));
            let standing = host.demand(&b, AttachMode::Durable).unwrap();
            wait_for_park(&host, &b, Demand::DuplicateRoot(conflict));
            wait_for_rest(&host, &a);

            unregister(&host, UnregisterParams::new(a.clone()))
                .expect("a parked entry nothing holds was refused");

            assert_eq!(
                entry_park(&host, &b),
                None,
                "the conflict outlived its alias"
            );
            wait_for_state(&host, &b, TrustState::Ready);
            drop(standing);
        }

        /// A refused unregistration leaves the registration that stood,
        /// served by the entry that stood — park and all.
        #[cfg(unix)]
        #[test]
        fn a_refused_unregistration_leaves_the_park_it_found() {
            let ops = Arc::new(FakeOps::default());
            let scratch = temp_base("unregister-refused-park");
            let root = scratch.root().join("root");
            std::fs::create_dir_all(&root).unwrap();
            let (a, b) = (VaultName::new("a").unwrap(), VaultName::new("b").unwrap());
            let host = quiet_host_over_roots(
            ...
            let refusal = unregister(&host, UnregisterParams::new(a.clone()))
            ...
            for name in [&a, &b] {
                assert_eq!(
                    ...
                    Some(Demand::DuplicateRoot(conflict.clone())),
                    "{name} lost the park a refused unregistration found"
                );
            }
        }
        ...
        /// A registry file that refuses the change leaves the registration
        /// that stood before standing, served by the entry that stood, and a
        /// demand attaches it.
        #[test]
        ...
        fn every_door_answers_held_while_an_unregistration_holds_the_lock() {
            ...
            *ops.retire_pause.lock().unwrap() = Some(Arc::clone(&pause));
            ...
            assert_eq!(
                ...
                [norn_wire::Attention::parked(
                    name.clone(),
                    norn_wire::ReasonCode::HostEntryHeld
                )]
            );
            ...
            assert_eq!(
                status_refused(&host, &status_params(&name)),
                ErrorDetail::unknown_vault(name.clone())
            );
            assert_eq!(rolled_up(&host).vaults(), 0);
            settle();
            assert_eq!(
                ops.attaches.load(Ordering::SeqCst),
                0,
                "a door attached the vault being unregistered"
            );
        }
        ...
        /// A name the registry file already records — another writer's
        /// registration since startup — is refused as taken, and the file
        /// keeps that writer's registration.
        #[test]
        fn a_name_the_registry_file_records_is_refused_and_the_file_keeps_it() {
            ...
            let theirs = RegistryEntry::new(
                name.clone(),
                VaultRoot::new("/tmp/norn-host-another-writers-root").unwrap(),
            );
            ops.registry
                .lock()
                .unwrap()
                .insert(name.clone(), theirs.clone());

            let refusal = register(&host, &name, scratch.root())
                .expect_err("a recorded name was registered over");
        ...
        /// entry in service and the registration changes open: the vault
        /// answers a demand, and the next unregistration goes through.
        #[test]
        fn a_retirement_that_unwinds_puts_the_entry_back_in_service() {
            ...
            assert!(unwound.is_err(), "the retirement did not unwind");
            assert_eq!(listed(&host), std::slice::from_ref(&name));
            attach_and_idle(&host, &name);
            ops.panic_in_retire.store(false, Ordering::SeqCst);
            unregister(&host, UnregisterParams::new(name.clone()))
        ...
        /// The registrations the host lists, ascending.
        fn listed_registrations(host: &Host<Arc<FakeOps>>) -> Vec<Registration> {
            host.vault_list(&ListParams::new()).registrations
        }

        /// The registration the entry serving `name` carries.
        fn serving(host: &Host<Arc<FakeOps>>, name: &VaultName) -> Registration {
            host.shared
                .entries
                .get(name)
                .expect("the vault is served")
        ...
        /// A host serving `registration` alone, with no ambient polling, and
        /// the fake's registry file recording it as the file a host starts
        /// from does.
        fn quiet_host_serving(
            ops: &Arc<FakeOps>,
            registration: &Registration,
        ) -> Host<Arc<FakeOps>> {
            ops.registry
        ...
        fn schema_source(path: &str) -> SchemaSource {
            SchemaSource::new(path).unwrap()
        }

        /// Attach `name` and answer the registration the attach was handed.
        fn attached_under(
            host: &Host<Arc<FakeOps>>,
            ops: &FakeOps,
            name: &VaultName,
            ...
            wait_for_state(host, name, TrustState::Ready);
            drop(lease);
            ops.attach_registrations
                .lock()
                .unwrap()
                .get(name)
                .cloned()
                .expect("the vault was attached")
        }

        /// A schema source an edit sets is the registration's from then on: the
        /// report echoes it, the file records it, the listing names it, and the
        /// attach that follows is handed it.
        #[test]
        ...
        /// A cleared schema source falls back to the in-vault default, in the
        /// file and in the attach that follows.
        #[test]
        fn a_cleared_schema_source_falls_back_to_the_in_vault_default() {
            let ops = Arc::new(FakeOps::default());
            let name = VaultName::new("notes").unwrap();
            let host = quiet_host_serving(
                &ops,
                &RegistryEntry::new(name.clone(), VaultRoot::new("/tmp/norn-host-set").unwrap())
                    .with_schema_source(schema_source("/tmp/norn-host-schemas/notes.yaml")),
            );
        ...
        /// A cleared watch backend falls back to the platform's native one.
        #[test]
        fn a_cleared_poll_backend_falls_back_to_the_native_watcher() {
            let ops = Arc::new(FakeOps::default());
            let name = VaultName::new("notes").unwrap();
            let host = quiet_host_serving(
                &ops,
            ...
            assert_eq!(report.registration.poll_backend, None);
            assert_eq!(recorded(&ops, &name), Some(report.registration.clone()));
            assert_eq!(attached_under(&host, &ops, &name), report.registration);
        }

        /// An edit that keeps every field answers the registration as it
        /// stands and what its entry publishes, writes nothing, and leaves the
        /// entry serving the name as it was.
        #[test]
        fn an_edit_keeping_every_field_answers_the_registration_and_writes_nothing() {
            ...
            assert!(
                ops.amendments.lock().unwrap().is_empty(),
        ...
        /// An edit to the values standing — the root spelled through a link to
        /// the directory it is, the schema source and backend it holds — is an
        /// edit that changes nothing.
        #[cfg(unix)]
        #[test]
        fn an_edit_to_the_values_standing_writes_nothing() {
            let ops = Arc::new(FakeOps::default());
            let scratch = temp_base("set-standing-values");
            let root = scratch.root().join("root");
            std::fs::create_dir_all(&root).unwrap();
            let link = scratch.root().join("link");
            std::os::unix::fs::symlink(&root, &link).unwrap();
            let name = VaultName::new("notes").unwrap();
            let source = schema_source("/tmp/norn-host-schemas/notes.yaml");
            let standing = RegistryEntry::new(
                name.clone(),
                VaultRoot::new(std::fs::canonicalize(&root).unwrap()).unwrap(),
            )
            .with_schema_source(source.clone())
            .with_poll_backend(PollBackend::Poll);
            let host = quiet_host_serving(&ops, &standing);
            ...
            assert_eq!(report.registration, standing);
            assert!(
                ops.amendments.lock().unwrap().is_empty(),
                "the file was written"
            );
        ...
        /// A root moved to a spelling through a link is stored, recorded,
        /// listed and attached at the directory the link reaches.
        #[cfg(unix)]
        #[test]
        fn a_root_moved_through_a_link_is_stored_at_its_directory() {
            let ops = Arc::new(FakeOps::default());
            let scratch = temp_base("set-root-link");
            let (old, new) = (scratch.root().join("old"), scratch.root().join("new"));
        ...
        /// A root the vault's own registration reaches under another spelling
        /// is no duplicate of it: the vault moves to the directory's canonical
        /// spelling.
        #[cfg(unix)]
        #[test]
        fn a_root_respelled_at_the_directory_it_reaches_is_no_duplicate_of_itself() {
            let ops = Arc::new(FakeOps::default());
            let scratch = temp_base("set-root-respelled");
            let root = scratch.root().join("root");
            std::fs::create_dir_all(&root).unwrap();
            let link = scratch.root().join("link");
            std::os::unix::fs::symlink(&root, &link).unwrap();
        ...
        /// A root another served vault already reaches is refused naming both,
        /// before anything is written — and the incumbent goes on serving it.
        #[cfg(unix)]
        #[test]
        fn a_root_moved_onto_another_served_root_is_refused_and_the_incumbent_goes_on_serving() {
            let ops = Arc::new(FakeOps::default());
            let scratch = temp_base("set-root-alias");
            let (theirs, ours) = (scratch.root().join("theirs"), scratch.root().join("ours"));
            let (served, name) = (
                VaultName::new("served").unwrap(),
                VaultName::new("notes").unwrap(),
            );
            let host = host_over_roots(
                Arc::clone(&ops),
                &[(&served, theirs.as_path()), (&name, ours.as_path())],
                2,
            );
            record_startup(&host, &ops, &name);
            ...
            assert_eq!(
                refusal.detail(),
                &ErrorDetail::duplicate_root(colliding(&name, &served))
            );
            ...
            assert_eq!(
                incumbent.completion(),
                Demand::State(TrustState::Ready),
                "the incumbent was parked by an edit that was refused"
            );
            assert_eq!(ops.detaches.load(Ordering::SeqCst), 0);
            drop(incumbent);
        }
        ...
        #[cfg(unix)]
        #[test]
        fn a_root_moved_to_no_readable_directory_is_refused_as_the_environment() {
            use std::os::unix::fs::PermissionsExt;

            let ops = Arc::new(FakeOps::default());
            let scratch = temp_base("set-root-unreadable");
            let file = scratch.root().join("file");
            std::fs::write(&file, b"not a vault").unwrap();
            let sealed = scratch.root().join("sealed");
            std::fs::create_dir_all(&sealed).unwrap();
        ...
        /// A name the set does not serve is unknown, and nothing is written.
        #[test]
        ...
        /// An entry something holds is refused, nothing is written, and it
        ...
        /// An entry standing on a park that nothing holds is refused in the
        /// park's own code, nothing is written, and the park stands.
        #[test]
        fn an_edit_of_a_parked_entry_is_refused_in_the_parks_code_and_the_park_stands() {
            ...
            assert_eq!(entry_park(&host, &name), Some(contended), "the park went");
            assert!(ops.amendments.lock().unwrap().is_empty());
            assert_eq!(recorded(&ops, &name), Some(standing.clone()));
            assert_eq!(serving(&host, &name), standing);
        }
        ...
        /// stood, served by the entry that stood, listed as it was, and a
        /// demand attaches it under that registration.
        #[test]
        ...
        /// While an edit holds the vault, every door answers the entry as held
        /// and schedules nothing, while the listing names the registration as
        /// it stood. Once the edit commits the listing names the edit, and the
        /// vault answers from the entry that serves it.
        #[test]
        fn every_door_answers_held_while_an_edit_is_under_way() {
            ...
            *ops.retire_pause.lock().unwrap() = Some(Arc::clone(&pause));
            ...
            let (answers, edited) = thread::scope(|scope| {
                ...
                let lease = host.demand(&name, AttachMode::Durable).unwrap();
                let answers = (
                    [lease.outcome().clone(), lease.completion()],
                    host.begin_read(&name).err(),
                    host.reload(&name).err(),
                    host.state(&name)
                        .map_err(|refusal| refusal.detail().clone()),
                ...
                drop(lease);
                drop(held);
                (answers, editing.join().expect("the edit ran"))
            });

            let (demands, read, reload, state, listing, status, roll_up) = answers;
            assert_eq!(status, Err(ErrorDetail::entry_held(name.clone())));
            assert_eq!((roll_up.vaults(), roll_up.parked()), (1, 1));
            assert_eq!(
                roll_up.attention(),
                [norn_wire::Attention::parked(
            ...
            assert_eq!(demands, [const { Demand::EntryHeld }; 2]);
            assert!(
                matches!(read, Some(ReadRefusal::NotServing(Demand::EntryHeld))),
                "a read answered {read:?}"
            );
            ...
            assert_eq!(
                (rolled_up(&host).vaults(), rolled_up(&host).unattached()),
                (1, 1)
            );
            settle();
            assert_eq!(
                ops.attaches.load(Ordering::SeqCst),
                0,
                "a door attached the vault being edited"
            );
        }

        /// A caller still holding the entry an edit replaced is answered as
        /// held — a demand and a status alike — records nothing and attaches
        /// nothing; asking again by name reaches the entry serving the edit,
        /// and the roll-up counts that entry alone.
        #[test]
        fn a_demand_or_status_holding_the_replaced_entry_is_answered_held() {
            ...
            let observed = replaced
                ...
                .map(|_| ());
            let lease = host
                .demand_entry(&name, Arc::clone(&replaced))
                .expect("the host is running");
            ...
            let roll_up = rolled_up(&host);
            assert_eq!((roll_up.vaults(), roll_up.unattached()), (1, 1));
            assert_eq!(roll_up.attention(), []);
            assert_eq!(status_of(&host, &name).registration, edited.registration);
            assert_eq!(lease.outcome(), &Demand::EntryHeld);
            assert_eq!(replaced.gate.lock().unwrap().demand_leases, 0);
            drop(lease);
            settle();
            assert_eq!(ops.attaches.load(Ordering::SeqCst), 0);
            assert_eq!(
                attached_under(&host, &ops, &name).poll_backend,
                Some(PollBackend::Poll)
            );
        }

        /// **The entry serving an edit starts with nothing its predecessor's
        /// attachments recorded**: no advisory, no store reading, no delivered
        /// engine section and no engine. `vault status` reports it unattached
        /// with none of them until its own attach publishes what that attach
        /// met.
        #[test]
        fn the_entry_serving_an_edit_reports_nothing_until_its_own_attach() {
            ...
            *ops.advisories.lock().unwrap() = vec![linked("away.md")];
            ...
            let status = status_of(&host, &name);
            assert_eq!(status.registration, edited.registration);
            assert_eq!(status.published, Published::state(TrustState::Unattached));
            assert_eq!(status.advisories, []);
            assert_eq!(
            ...
            let _lease = host.demand(&name, AttachMode::Durable).unwrap();
            wait_for_state(&host, &name, TrustState::Ready);
            assert_eq!(status_of(&host, &name).advisories, [link]);
        }

        /// An amendment that unwinds leaves the entry that stood in service and
        /// the registration changes open: the vault answers a demand, and the
        /// next edit goes through.
        #[test]
        fn an_amendment_that_unwinds_puts_the_entry_back_in_service() {
            ...
            let report = set(&host, edit).expect("the vault is edited after the unwound attempt");
            assert_eq!(report.registration.poll_backend, Some(PollBackend::Poll));
        }

        /// A job the replaced entry left in the channel reaches nothing: the
        /// entry serving the edit starts past its epoch, and the worker that
        /// reaches the job runs nothing over it.
        ///
        /// The one worker is held on another vault's attach, so the first
        /// entry's rebuild waits in the channel; an identity refusal moves that
        /// entry past the job, which leaves the job queued and the entry
        /// holding nothing, and the registry park the refusal raised is
        /// withdrawn as a demand withdraws it, so the edit finds no park.
        #[test]
        fn a_job_the_replaced_entry_left_queued_reaches_nothing() {
            let ops = Arc::new(FakeOps::default());
            let scratch = temp_base("set-stale-job");
            let (busy_root, root) = (scratch.root().join("busy"), scratch.root().join("root"));
            let (busy, name) = (
                VaultName::new("busy").unwrap(),
                VaultName::new("notes").unwrap(),
            ...
            {
                let entry = host.shared.entries.get(&name).unwrap();
                entry.gate.lock().unwrap().retire_registry_parks();
            }
            wait_for_rest(&host, &name);
            ...
            let started = successor.gate.lock().unwrap().claim.epoch();
            ops.block_attach.store(false, Ordering::SeqCst);
            ops.attach_release.store(true, Ordering::SeqCst);
            wait_for_state(&host, &busy, TrustState::Ready);
            settle();

            assert!(
                started > stale,
                "the entry serving the edit started at {started}, not past the queued job's {stale}"
            );
            assert_eq!(
                ops.rebuilds.load(Ordering::SeqCst),
                0,
                "the queued rebuild ran over the entry serving the edit"
            );
            assert_eq!(host.state(&name), answered(TrustState::Unattached));
            assert_eq!(
                attached_under(&host, &ops, &name).poll_backend,
