...
//!   trust-transition arms nothing yet reaches.
//! - [`manifest`] — **what certified the candidate**: one digest over the lanes,
//!   the toolchain, the resolved dependency graph, the inventory, the
//!   comparator, the fault seams and the authored bounds. Two runs of a suite
//!   that changed underneath them are two suites, and this is the value that
//!   says so.
...
//!   classification with a typed reason where the run does not count.
//!
