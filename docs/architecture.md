...
having failed. The same discipline carries the effect surfaces `norn-fs` owns, where **two
sibling fault seams are each widened once, at their own boundary** — the write protocol's at
the public write entry point, the watcher's at watch establishment — under one feature, read
from the environment a process is started with. So process death at each named stage of a
...
reports. Both seams append to one record file and are told apart by the `seam` field a
record carries.
