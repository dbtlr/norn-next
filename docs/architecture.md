...
8. **Two vault-effect seams, and only two.** In the shipped product, `norn-fs` and
   ...
   seam rather than a third one beside it: nothing but the store and the engine's own
   sidecar link it, so the effect still has one owner and one way in. `norn-semantic`
   stays inside the seam the same way: it reads the derived database only through the
   store's feed-read handle, and its sidecar — a second derived database beside the
   first, in the same per-vault derived directory — runs on `norn-db` like every norn
   database does. Derivation is
