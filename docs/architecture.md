...
retains a typed file-and-stage error, and leaves a `Ready` vault `Ready`. A successful
candidate clears that error. The host dispatches the vault identity and each registered
...
walk is hash-authoritative: it reads every Markdown path that needs derivation. A
