...
none, and a path climbing above the root names none. A `vault://` stem is a path from the
vault root, read the same way. A same-document anchor names its own document. A link is
...
and a path's under the vault path it names; each raw and with ASCII case folded. A link
