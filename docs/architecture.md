...
- **Where a write is staged.** `norn-fs` stages in the shadow home under the data root
  ...
  **Exclusion is a membership boundary**: an excluded place holds no rows, and a heal
  that ranges over rows standing under an excluded root prunes them.
