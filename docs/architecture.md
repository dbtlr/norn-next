...
13. **The orchestrator is protocol-blind.** `norn-host` depends on no protocol or serving
    ...
    rather than a code invented at a call site. **`HostError` is the one channel that is not
