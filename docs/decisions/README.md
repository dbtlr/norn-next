...
| [0004](0004-two-tier-measurement-and-authored-baselines.md) | Counts gate per PR and clocks only trend in soak, where a wall-clock bar is a sanity ceiling rather than a gate; a trend's whole memory is a checked-in authored baseline — peak memory a ratchet — moved only by a reviewed edit; and a bar's subject is whatever exists, with the slot naming the layer that will hold it. |
