...
behind every warm job that holds the store while trust stays `Ready`, and the timing-barred
query shapes would measure orchestration where the acceptance contract bars SQL. Wire reads
...
instant; the read itself runs outside the lock. A snapshot sees the last committed
...
assumption — is what mints more readers through the same seam. A reader never concludes a
...
running outside the entry's lock is coming back — and it buys the read exactly one thing:
**an idle reap is not scheduled while the read runs. Teardown never waits for a read.** A
...
it holds, and that the file behind that handle outlives the teardown is carried by the
handle alone — an obligation deliberately left unstated until the read path lands and
prices it.
...
read holds: nothing. A leg's pin defers refusal and destruction only because the leg is
out with the entry's coverage and the give-back is the leg's own end; a read holds no
coverage, so at those sites the entry is still holding its own and the refusal or the
destruction is itself what reaches detach. Granting a read the leg's veto would put reader
count in front of exactly the moves that must make progress — a conflict being refused, a
host coming down, a maintainership already lost — while buying nothing the shared handle
...
read shapes are what keep the pin short. The corrected rule prices one more: a read that a
